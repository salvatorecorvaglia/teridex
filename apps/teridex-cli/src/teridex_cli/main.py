...
def version() -> None:
    ...
    from teridex_core import __version__
...
def run_query(
    ...
    limit: Annotated[
        int, typer.Option("--limit", min=1, help="Max rows to print.")
    ] = 200,
) -> None:
    ...
    async def _go() -> int:
        ...
        try:
            ...
            if table is not None:
                console.print(table)
            console.print(
                f"[dim]{run_handle.rows_emitted} row(s) in "
                f"{run_handle.duration_ms or 0:.1f} ms[/]"
            )
            return 0
        finally:
            await bus.close()
...
def tui(
    ...
    config_path: Annotated[
        str, typer.Option("--config", help="Path to config TOML.")
    ] = "",
) -> None:
